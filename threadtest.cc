...
void read(int id){
	for (int i = 0; i <MSize; i++){
		if(mailbox[id][i] != 9999){
			printf("\n%s received a message with content: \"%s\"",
	...
	if(totalSent >= NOMail)
		...
		while(isFull(mailTo)){
			if(deadlockcounter<5){
				...
				printf("\nThis part just resolves deadlock");
				deadlockcounter = 0;
				int temp = mailTo;
				mailTo = Random()%NOPeople;
				while(mailTo == id && mailTo == temp){
					mailTo = Random()%NOPeople;
				}
			}
		}
		for(int i = 0; i<MSize;i++){
			if(mailbox[mailTo][i] == 9999){
				...
				printf("\n%s just sent a message to Person #%d",
...
void PostOffice(int id){
	while(totalSent < NOMail){
		...
		printf("\n%s officially leaves the post office", currentThread->getName());
	}
	//Begin Changes By Derek Murphy
}
void PostOfficeS(int id){
	mailAccessS = new Semaphore *[NOPeople];
	for(int i=0; i<NOPeople; i++){
		mailAccessS[i]= new Semaphore("what", NOMail);
	}
	while(totalSent < NOMail){
		printf("\n%s enters the post office", currentThread->getName());
